...
def test_url_already_exists(client, short_python_url):
    try:
        ...
        raise AssertionError(
            'Вызывайте исключение при попытке создания ссылки с ',
            'коротким именем которое уже занято',
        )
    assert got.status_code == 400, (
...
def test_len_short_id_api(client):
    ...
    assert got.status_code == 400, (
        ...
        'код статуса ответа должен быть 400'
    )
    assert list(got.json.keys()) == ['message'], (
        'При POST запросе в теле которого указана короткая ссылка более 16 символов '
...
def test_len_short_id_autogenerated_api(client):
    ...
    assert len(unique_id.short) == 6, (
        'При POST запросе в теле которого не укана короткая ссылка '
